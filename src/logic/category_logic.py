...
from src.utils.exceptions import CategoryIdNotFoundException, CategoryLanguageIdInvalidException
...
def create_or_update(category_dict: dict) -> Category:
    ...
    account_logic.check_if_admin_privilege(current_account, category_dict['language_id'])
    if category_dict['id']:
...
def delete(id: int):
    ...
    if category:
        current_account = account_logic.get_current_account()
        account_logic.check_if_admin_privilege(current_account, category.language_id)
...
def _create(category_dict: dict, current_account: Account):
    parent_category = Category.find_by_id(category_dict['parent_id'])
    if parent_category:
        if category_dict['language_id'] != parent_category.language_id:
            raise CategoryLanguageIdInvalidException([category_dict['language_id']])
    category = Category()
