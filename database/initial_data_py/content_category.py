from datetime import datetime

from src.models.content.category import Category
from src.models.system.language import Language

categories = [
    ['Maths', 'Maths is the queen of all sciences.'],
    ['Geography', 'Learn all the countries and capitals.'],
    ['Chemistry', 'Be careful not to blow anything up.'],
    ['Biology', 'All the squishy parts.']
]
lang = Language.find_by_code('en')
for cat in categories:
    category = Category(cat[0], cat[1], lang.id)
    category.created_date = datetime(2012, 12, 12)
    category.save_to_db()
